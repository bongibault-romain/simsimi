...
export async function add(sentence: string, answer: string, emotion: Emotion | null, authorId: string | null) {
...
export async function getAll(question: string): Promise<Answer[]> {
  ...
  if (matchingSentences.bestMatch.rating > 0.3) {
    ...
    const selected = result.map((r) => r.target).flat();

    let answers: Answer[] = [];

    for (let i = 0; i < selected.length; i++) {
      const selectedQuestion = rawQuestion.find(q => q.message === Buffer.from(selected[i], "utf8").toString("base64"));
      
      // eslint-disable-next-line no-await-in-loop
      answers = [...answers, ...(await knex.select("*").from("answers").where({
